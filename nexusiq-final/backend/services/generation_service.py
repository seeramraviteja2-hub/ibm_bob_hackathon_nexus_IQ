...
async def start_course_generation(
    ...
    if not files:
        raise AppException(detail="At least one file required", status_code=400)
    ...
    if mode == "new_project":
        ...
        f = files[0]
        if f.filename.rsplit(".", 1)[-1].lower() not in ("pdf", "docx", "doc", "txt"):
            raise AppException(detail=f"Unsupported file type: {f.filename}", status_code=400)
        obj = f"specs/{course_id}/{job_id}/{f.filename}"
        await handler.upload_file(bucket, obj, await f.read(), f.content_type or "application/octet-stream")
        input_data = {"object_name": obj, "bucket": bucket, "filename": f.filename}
        ...
        for f in files:
            ...
            obj = f"code/{course_id}/{job_id}/{f.filename}"
            await handler.upload_file(bucket, obj, await f.read(), "text/plain")
            file_paths.append({"bucket": bucket, "object_name": obj, "file_path": f.filename})

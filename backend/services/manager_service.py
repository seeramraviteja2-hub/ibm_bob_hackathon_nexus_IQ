...
# ── New endpoints: list employees, get assigned, unassign, course detail ──────
...
async def get_assigned_employees(course_id: str, manager_id: str, db: AsyncSession) -> list[dict]:
    """Return employees assigned to a course."""
    course = await db.scalar(
        select(Course).where(Course.id == uuid.UUID(course_id), Course.manager_id == uuid.UUID(manager_id))
    )
    if not course:
    ...
    return [
        {
            ...
            "email": a.employee.email if a.employee else "",
            "status": a.status.value,
        }
        for a in assignments
    ]
...
async def unassign_employee(course_id: str, employee_id: str, manager_id: str, db: AsyncSession) -> dict:
    ...
    ec = await db.scalar(
        select(EmployeeCourse).where(
            EmployeeCourse.course_id == uuid.UUID(course_id),
            EmployeeCourse.employee_id == uuid.UUID(employee_id),
        )
    )
    if ec:
        await db.delete(ec)
        await db.commit()
    return {"unassigned": bool(ec)}


async def get_course_detail(course_id: str, manager_id: str, db: AsyncSession) -> dict:
    """Return course details for manager view."""
    course = await db.scalar(
        select(Course)
        .where(Course.id == uuid.UUID(course_id), Course.manager_id == uuid.UUID(manager_id))
        .options(selectinload(Course.modules))
